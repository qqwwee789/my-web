...
var S = {
  init: function () {
    ...
    if (i !== -1) {
      ...
      // 在 1 后面增加一个空字符，用于触发消失动画
      S.UI.simulate('黄晓小|要开心|准备好了吗|3|2|1| ');
    }
    ...
    if (!max || (!reverse && currentAction < max) || (reverse && currentAction > 0)) {
      interval = setInterval(function () {
        ...
        if ((!reverse && max && currentAction === max) || (reverse && currentAction === 0)) {
          clearInterval(interval);
          // 当序列执行完毕时，检查是否需要触发弹窗
          if (sequence.length === 0 && window.startPopups) {
              // 延迟一小段时间确保最后的粒子消失动画完成
              setTimeout(function() {
                  window.startPopups();
                  // 播放音乐
                  var audio = document.getElementById('wish-audio');
                  if (audio) {
                      audio.play().catch(function(e) {
                          console.log("Audio play failed, might need user interaction:", e);
                      });
                  }
              }, 1000);
          }
          ...
          timedAction(function (index) {
            if (index === 0) {
              if (sequence.length === 0) {
                ...
                // 倒计时结束且没有后续序列，触发弹窗和音乐
                setTimeout(function() {
                    if (window.startPopups) window.startPopups();
                    var audio = document.getElementById('wish-audio');
                    if (audio) audio.play();
                }, 1000);
              } else {
